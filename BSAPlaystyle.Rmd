...
# write.csv(data.frame(good_list), "./data/goodplayers.csv")

df_busts |> pull(player)
bust_list <- df_busts |> pull(player)

# write.csv(data.frame(bust_list), "./data/bustplayers.csv")
...
df_cbb.pca <- PCA(df_cbb.sample |> select(-games, -fg3_pct_per_g), scale.unit = TRUE, graph = FALSE)
